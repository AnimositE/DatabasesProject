...
class Database:
    ...
    def getDiverProfile(self, diverid):
        # TODO: pull meets
        self.cursor.execute("SELECT fName, lName, email, age, Schools.name, division, favoriteDive FROM Divers JOIN Profiles ON Divers.id = diverID JOIN Schools ON schoolID = Schools.id WHERE Divers.id = %s;",[diverid])
        return self.cursor.fetchall()
